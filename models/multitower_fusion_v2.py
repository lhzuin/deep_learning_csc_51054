...
class MultiTowerFusionV2(nn.Module):
    """
    Late-fusion model:
    ...
    def __init__(
        ...
        # --- description tower architecture ---
    ...
    def forward(self, batch, return_parts=False):
        ...
        # ---- Fast path: cached fused features ----
        if "fused" in batch:
            fused = batch["fused"].to(device)   # (B, fusion_in_dim)
            logp_fused = self.fusion_head(fused)
            if return_parts:
                return {"logp_fused": logp_fused}
            return logp_fused

        # ---- Normal path: compute towers every time ----
        # ---- Meta tower ----
    ...
    def _build_param_groups(self):
        ...
        groups = {
            ...
            "meta": list(self.meta_tower.parameters()),
            # if later you want to fine-tune towers, you can add more groups here
        }

        if hasattr(self.tweet_tower, "get_param_groups"):
            for name, params in self.tweet_tower.get_param_groups().items():
                groups[f"tweet_{name}"] = params

        # Desc tower groups: desc_mlp, desc_classifier, desc_text_lora_all, desc_enc_last2
        if hasattr(self.desc_tower, "get_param_groups"):
            for name, params in self.desc_tower.get_param_groups().items():
                groups[f"desc_{name}"] = params
